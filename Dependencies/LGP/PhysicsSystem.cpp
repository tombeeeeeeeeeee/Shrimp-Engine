...
void LGP::PhysicsSystem::CollisionPhase(std::unordered_map<unsigned int, PhysicsComponent>& bodies, 
...
void LGP::PhysicsSystem::IntegrationStep(std::unordered_map<unsigned int, PhysicsComponent>& bodies, 
    std::unordered_map<unsigned int, TransformComponent>& transforms, 
    float deltaTime)
{
...
void LGP::PhysicsSystem::SetCollisionLayerMask(int layerA, int layerB, bool state)
{
...
void LGP::PhysicsSystem::SetCollisionLayerMask(int layer, unsigned int bitMask)
{
...
void LGP::PhysicsSystem::ShapeCollisionCheck(PhysicsComponent& aPhysics, const TransformComponent& aTransform, 
	PhysicsComponent& bPhysics, const TransformComponent& bTransform)
{
    if ((layerMasks[aPhysics.collisionLayer] & (1 << bPhysics.collisionLayer)) == 0)
        return;
...
LGP::CollisionPacket LGP::PhysicsSystem::GJK(Shape a, Shape b, glm::mat4 aTransform, glm::mat4 bTransform)
{
...
std::vector<glm::vec3> LGP::PhysicsSystem::ToVector(Shape shape)
{
...
glm::vec3 LGP::PhysicsSystem::CalculateSupport(std::vector<glm::vec3>& a, float aRadius, std::vector<glm::vec3>& b, float bRadius, glm::vec3 direction)
{
    return SupportFunction(direction, a, aRadius) - SupportFunction(-direction, b, bRadius);
}

glm::vec3 LGP::PhysicsSystem::SupportFunction(glm::vec3 dir, std::vector<glm::vec3>& verts, float radius)
{
    float max = -FLT_MAX;
    int index = 0;
...
glm::vec3 LGP::PhysicsSystem::SupportFunction(glm::vec3 dir, std::vector<glm::vec3>& verts, float radius, int& index)
{
    float max = -FLT_MAX;
    index = 0;
    for (int i = 0; i < verts.size(); i++)
...
LGP::GJKEvolution LGP::PhysicsSystem::AddSupportToSimplex(std::vector<glm::vec3>& simp, glm::vec3 vert, glm::vec3 dir)
{
    simp.push_back(vert);
    return glm::dot(dir, vert) >= 0 ? GJKEvolution::evolving : GJKEvolution::notIntersecting;
}

LGP::GJKEvolution LGP::PhysicsSystem::EvolveSimplex(std::vector<glm::vec3>& simp, glm::vec3& direction)
{
    switch (simp.size())
    {
    case 2:
...
void LGP::PhysicsSystem::EPA(std::vector<glm::vec3>& simp, CollisionPacket& collision, 
...
std::vector<glm::vec4> LGP::PhysicsSystem::GetFaceNormals(std::vector<glm::vec3>& polytope, std::vector<int> faces, int& face)
{
...
void LGP::PhysicsSystem::AddIfUniqueEdge(std::vector<std::pair<int, int>>& uniqueEdges, std::vector<int> edge, int aVert, int bVert)
{
...
void LGP::PhysicsSystem::CalculateCollsionPoint(std::vector<glm::vec3>& aVerts, Shape& aShape, std::vector<glm::vec3>& bVerts, Shape& bShape, glm::vec3 normal, glm::vec3& contactPoint)
{
    contactPoint = {0,0,0};
...
void LGP::PhysicsSystem::AddFaceVert(glm::vec3 normal, int originIndex, int currIndex, std::vector<glm::vec3>& worldSpace, Shape shape, std::vector<int>& verts)
{
...
void LGP::PhysicsSystem::Resolution(CollisionPacket& collision)
{
...
glm::vec3 LGP::PhysicsSystem::AddDepen(glm::vec3 newDepen, glm::vec3& currDepen)
{
    glm::vec3 nextDepen;
...
void LGP::PhysicsSystem::Integration(PhysicsComponent& body, TransformComponent& transform, float deltaTime)
{
...
glm::quat LGP::PhysicsSystem::OrthonormalizeOrientation(const glm::mat4 rotation)
{
